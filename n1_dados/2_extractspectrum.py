...
if os.path.exists(darkframe_dir)==False:
    print(bcolors.FAIL+"   [ERROR] Direcory to Darkframes don't exist! "+bcolors.ENDC)
    print(bcolors.FAIL+"   [ERROR] ABORT"+bcolors.ENDC)
    sys.exit()
if os.path.isfile(darkframe_dir)==True:
...
#plot idents to figure
    for line in lines:
        liste=line.split()
        if len(liste) == 1:
            print(bcolors.WARNING+"     [WARNING] Broken identification found as '"+line+"', must consist of [wavelenght(s) + name]. I will skip this one."+bcolors.ENDC)
            continue
        try:
            float(liste[0])
        except ValueError:
            print(bcolors.WARNING+"     [WARNING] Broken identification found as '"+line+"', first entry not a number. I will skip this one."+bcolors.ENDC)
            continue
