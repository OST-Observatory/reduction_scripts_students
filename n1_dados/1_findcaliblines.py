    ...
    if event.button == 1:
        ...
        if len(linelist_tmp) > 0:
            ...
            for i in range(len(maxindexlist)):
                distance.append(np.sqrt(((event.xdata-maxindexlist[i])/len(spec))**2+((event.ydata-maxvaluelist[i])/((max(spec)+0.1*max(spec))*(16/9)))**2))
...
#print(m1,m2,m3,b)
print(bcolors.OKGREEN+"   Fit: "+'y = '+str('%.2E' % Decimal(m1))+" x^3$ + "+str('%.2E' % Decimal(m2))+" x^2$ + "+str('%.2E' % Decimal(m3))+"x + " + str('{:7.2f}'.format(b))+bcolors.ENDC)


fig2.suptitle('$\lambda$ = '+str('%.2E' % Decimal(m1))+r" $\times$ pxl$^3$ + "+str('%.2E' % Decimal(m2))+r" $\times$ pxl$^2$ + "+str('%.2E' % Decimal(m3))+r" $\times$ pxl + " + str('{:7.2f}'.format(b)),fontsize=15)
